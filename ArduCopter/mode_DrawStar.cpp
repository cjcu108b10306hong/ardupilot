...
{
    path_num = 0;
    generate_path();  // 生成五角星航線
    pos_control_start();  // 開始位置控制
    return true;
}
...
{
  if (path_num < 6) {  // 五角星航線尚未走完
    if (wp_nav->reached_wp_destination()) {  // 到達某個端點
      path_num++;
      wp_nav->set_wp_destination(path[path_num], false);  // 將下一個航點位置設置為導航控制模組的目標位置
    }
  }
...
void ModeDrawStar::pos_control_run()  // 注意，此函數直接從mode_guided.cpp中複製過來，不需要該其中的內容
{
